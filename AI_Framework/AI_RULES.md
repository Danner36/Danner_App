...
- Inject the selected coordinates into the verification WebView's browser Geolocation API. Do not modify device GPS, launch a separate mock-location app, or claim that phone location changed. TV Location does not read GPS. Android Cast declares `ACCESS_FINE_LOCATION` and, on API 29–32, requests it when Send to TV runs so Chromecast discovery can run.
