...
| Latest published family tag | `v1.4.11`. Family phones on older tags need `v1.4.9` or newer for the Cyclones tile |
...
Source under `app/cyclones/` is implemented. Remaining owner work is later listing-needle inspect after the first football listing row. Family release is `v1.4.11`. Do not fold Cyclones into Patriots.
