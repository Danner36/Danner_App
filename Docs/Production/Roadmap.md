...
| Patriots dashboard | Implementation complete | ESPN-backed leftover preseason, regular-season, and playoff schedule, regular-season W–L–T, today/live/Final-recap featured card, football scoreboard, Time TBA, and the same Watch timing as Guardians. Device confirmation is still required |
...
| Direct device distribution | Complete | GitHub release `v1.3.3` contains the installable Android APK, SideStore iPhone IPA, iPhone setup guide, and SHA-256 checksum file; later `v*` tags use the same gated two-platform release workflow and also publish `version-manifest.json` and `sidestore-source.json` |
