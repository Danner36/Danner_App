...
| Danner app hub | Complete | Logo-only two-row 2×2 hub: Guardians and Patriots on row 1, Cyclones under Guardians, YouTube TV under Patriots |
...
| Cyclones dashboard | Implementation complete | ESPN-backed leftover football, men's basketball, and women's basketball for Iowa State team 66, three regular-season records, one featured card across sports, football and basketball boards, Time TBA, Chicago official dates, elimination status, and the same Watch timing as Patriots. Device confirmation is still required |
| Cyclones authorized playback | Implementation complete | Root `cyclones_streams.json` holds the six-field schema plus required `sport` and an inactive copyable example. Entries match America/Chicago official dates, game number 1, and sport |
| Cyclones Get video | Implementation complete | Phone POSTs `{ pin, module: "cyclones", sport }` to the shared Worker, which starts `cyclones-stream-pipeline.yml`. Live `/streams?module=cyclones` returned 200 after the 2026-09-04 Worker deploy and `cyclones_streams.json` on `main`. Owner secret `CYCLONES_STREAM_PIPELINE_CONFIG` is set. Family phones need `v1.4.9` or newer for the Cyclones tile |
...
| Direct device distribution | Complete | GitHub release `v1.4.10` contains the installable Android APK, SideStore iPhone IPA, iPhone setup guide, SHA-256 checksum file, `version-manifest.json`, and `sidestore-source.json`; later `v*` tags use the same gated two-platform release workflow |
