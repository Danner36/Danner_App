...
| Cyclones Get video | Implementation complete | Phone POSTs `{ pin, module: "cyclones", sport }` to the shared Worker, which starts `cyclones-stream-pipeline.yml`. Live `/streams?module=cyclones` returned 200 after the 2026-09-04 Worker deploy and `cyclones_streams.json` on `main`. Owner secret `CYCLONES_STREAM_PIPELINE_CONFIG` is set. Family phones need `v1.4.9` for the Cyclones tile |
...
| Direct device distribution | Complete | GitHub release `v1.4.9` contains the installable Android APK, SideStore iPhone IPA, iPhone setup guide, SHA-256 checksum file, `version-manifest.json`, and `sidestore-source.json`; later `v*` tags use the same gated two-platform release workflow |
