...
| Direct device distribution | Complete | GitHub release `v1.4.2` contains the installable Android APK, SideStore iPhone IPA, iPhone setup guide, SHA-256 checksum file, `version-manifest.json`, and `sidestore-source.json`; later `v*` tags use the same gated two-platform release workflow |
