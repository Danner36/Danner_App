export type CyclonesSport =
  | 'football'
  | 'mens-basketball'
  | 'womens-basketball';

function isCyclonesSport(value: unknown): value is CyclonesSport {
  return (
    value === 'football' ||
...
export type CyclonesPlaybackKind = 'direct' | 'web' | 'youtube';

export type AuthorizedCyclonesStream = {
  allowInsecureHttp: boolean;
  gameDates: string[];
  gameNumbers: number[];
  kind: CyclonesPlaybackKind;
  sport: CyclonesSport;
  trustedHosts: string[];
  url: string;
};

export type PlayableCyclonesStream = AuthorizedCyclonesStream & {
  allowedNavigationHosts: string[];
  playbackUrl: string;
};

export type CyclonesGameIdentity = {
  gameNumber: number;
  officialDate: string;
  sport: CyclonesSport;
};
...
export function playableCyclonesStream(
  stream: AuthorizedCyclonesStream,
): PlayableCyclonesStream | undefined {
  ...
  try {
    ...
    if (stream.kind === 'web') {
      ...
      if (
        trustedHosts.some((host) => !host || !/^[a-z0-9.-]+$/.test(host))
      ) {
        return undefined;
      }
...
function streamFromUnknown(
  ...
  const stream = candidate as Partial<AuthorizedCyclonesStream>;
  if (
    Object.keys(candidate).some((key) => !STREAM_FIELDS.has(key)) ||
    !isCyclonesSport(stream.sport) ||
  ...
  return playableCyclonesStream(stream as AuthorizedCyclonesStream);
}

export function cyclonesStreamsFromDocument(
  value: unknown,
): PlayableCyclonesStream[] | undefined {
...
export function authorizedStreamsForGame(
  streams: PlayableCyclonesStream[],
  game: CyclonesGameIdentity,
): PlayableCyclonesStream[] {
  return streams.filter(
    (stream) =>
      stream.sport === game.sport &&
