...
- When the featured game is inside the 15-minute video window and no valid matching URL is in that file, the card shows **Get video**. That button calls the Cloudflare Worker, which starts the GitHub Actions extract pipeline. The screen polls the GitHub file until a valid stream appears, then shows Play. Get video is hidden in the local test harness.
