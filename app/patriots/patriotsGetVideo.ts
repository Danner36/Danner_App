...
export function liveStreamsUrl(): string | undefined {
  ...
  return `${GET_VIDEO_URL.replace(/\/$/, '')}/streams?module=patriots`;
}
...
export async function requestGetVideo(): Promise<void> {
  ...
  try {
    response = await fetch(getVideoEndpoint(), {
      ...
      body: JSON.stringify({ module: 'patriots', pin: FAMILY_PIN }),
...
export async function pollForStream(
  game: PatriotsGameIdentity,
  fetchSources: () => Promise<PlayablePatriotsStream[]>,
  options?: {
    intervalMs?: number;
    timeoutMs?: number;
  },
): Promise<PlayablePatriotsStream | undefined> {
