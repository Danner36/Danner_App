...
const uiHas = async (needle) => {
  const labels = await dumpLabels();
  return labels.includes(needle);
};

const agentLog = async (hypothesisId, location, message, data) => {
  const payload = {
    sessionId: 'af2248',
    runId: 'emulator-get-video',
    hypothesisId,
    location,
    message,
    data,
    ...
    const server = createServer(async (request, response) => {
      ...
      if (
        ...
        try {
          ...
          pin = JSON.parse(Buffer.concat(chunks).toString('utf8')).pin ?? '';
        } catch {
          pin = '';
        }
        if (pin !== familyPin) {
        ...
        response.end(
          `${JSON.stringify({ message: 'Pipeline started.', ok: true })}\n`,
        );
        return;
      }
...
const harnessUrl = `http://10.0.2.2:${port}/guardians?scenario=live`;
const sourcesUrl = `http://10.0.2.2:${port}/guardians-sources.json`;
...
try {
  ...
  await waitFor('Guardians tile', 180_000, async () => {
  ...
  await agentLog('A', 'run-get-video.mjs:tapped', 'tapped Get video', {
    publishedAt,
    sourcesReady: sourcesReady(),
  });

  await sleep(4_000);
  const duringStale = await dumpLabels();
  await agentLog('A', 'run-get-video.mjs:during-stale', 'UI during stale window', {
    duringStale,
    sourcesReady: sourcesReady(),
  });
