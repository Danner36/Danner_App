...
if (!new Set(['delayed', 'final', 'live', 'ready', 'today']).has(scenario)) {
  throw new Error(`Unknown Patriots harness scenario: ${scenario}`);
}
const harnessUrl = `http://10.0.2.2:8108/patriots?scenario=${scenario}`;
const sourcesUrl = 'http://10.0.2.2:8108/patriots-sources.json';
...
const waitForServer = async () => {
  ...
  throw new Error('Patriots harness did not start.');
};
