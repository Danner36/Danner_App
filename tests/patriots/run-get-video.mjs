...
const officialDate = easternDateString(new Date());
const publishedStream = {
  ...
  kind: 'web',
  trustedHosts: [],
  url: 'https://example.com/patriots-stream',
};
let publishedAt = 0;
  ...
  new Promise((resolve) => {
    const server = createServer(async (request, response) => {
          ...
          JSON.stringify({
            liveGame: {
              ...
              scoreboard: {
                ...
                down: 2,
                distance: 7,
                situation: '2nd & 7 at SEA 33',
                possessionTeamId: 17,
                away: { points: 14, quarters: [7, 7] },
                home: { points: 10, quarters: [3, 7] },
              },
            },
            losses: 0,
            ties: 0,
      ...
      if (
        ...
        if (
          requestUrl.pathname === '/streams' &&
          requestUrl.searchParams.get('module') !== 'patriots'
        ) {
      ...
      if (request.method === 'POST' && requestUrl.pathname === '/get-video') {
        ...
        response.end(
          `${JSON.stringify({ message: 'Pipeline started.', module: 'patriots', ok: true })}\n`,
        );
        return;
      }
...
const harnessUrl = `http://10.0.2.2:${port}/patriots?scenario=live`;
const sourcesUrl = `http://10.0.2.2:${port}/patriots-sources.json`;
...
const env = {
  ...
  EXPO_PUBLIC_PATRIOTS_SOURCES_URL: sourcesUrl,
  EXPO_PUBLIC_PATRIOTS_TEST_URL: harnessUrl,
};
...
try {
  ...
  await waitFor('Patriots tile', 180_000, async () => {
